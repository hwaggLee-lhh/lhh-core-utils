...
import com.lhh.format.cls.ModelMethodUtils;
import com.lhh.format.cls.ModelUtils;
import com.lhh.format.lang.DateTimeUtils;
...
public class SQLGenerate {
	public static String generateInsertSql(String[] columnIndex,String tableName, Object model) {
		...
		sb.append(tableName);
		sb.append("(");
		Map<String, Method> methodByPro = ModelMethodUtils.getGetMethod(model);
		...
		for (int i = 0; i < columnIndex.length; i++) {
			...
			if(getObj instanceof String || 
				...
				values.append("'");
				values.append(DateTimeUtils.date2StrDate((Date)getObj, DateTimeUtils.FORMAT_yyyy_MM_dd_HH_mm_ss));
				values.append("'");
			} else {
				values.append(getObj);
			} 
	...
	public static String generateInsertBatchSql(String[] columnIndex,String tableName, Object model, List<? extends Object> models) {
		...
		for(int i=0; i<models.size(); i++){
			...
			for (int j = 0; j < columnIndex.length; j++) {
				...
				Object getObj = ModelUtils.invokeGetMethod(models.get(i), method);
	            if (getObj == null) continue;
