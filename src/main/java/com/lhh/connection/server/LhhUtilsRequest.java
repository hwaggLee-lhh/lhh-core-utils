...
public class LhhUtilsRequest {
	...
	/**
	 * 根据当前请求的特征，判断该请求是否来自手机终端，主要�?��特殊的头信息，以及user-Agent这个header
	 * 
	 * @param request
	 *            http请求
	...
	/**
	 * 根据当前请求的特征，判断该请求是否来自手机终端，主要�?��特殊的头信息，以及user-Agent这个header
	 * 目前只控�?user-Agent中只控制java和phone)或�?如果req中包�?Mozilla/5.0)则直接默认为false
	 * @param request
	 *            http请求
	 * @return 如果命中手机特征规则，则返回对应的特征字符串
	 */
	public static boolean isMobile(HttpServletRequest request) {
		...
		if(StringUtils.isNotBlank(userAgent)){
			if( userAgent.toLowerCase().contains("mozilla/5.0"))return false;
			if( userAgent.contains("iPhone") || userAgent.contains("java")){
				return true;
			}
		}
		return false;// false pc true shouji
	 ...
	 * @return
	 */
	public static List<Object> getRequestData(HttpServletRequest request){
		...
		list.add(request.getRequestURL());
		list.add(request.getRequestURI());
		list.add(request.getQueryString());
		list.add(request.getRemoteAddr());
		list.add(request.getRemoteHost());
		list.add(request.getRemotePort());
